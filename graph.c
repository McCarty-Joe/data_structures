...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    /*Add a link to the front of the deque*/
    addFrontCirListDeque(queue, source);

    /*Loop while the deque is not empty*/
    while(!isEmptyCirListDeque(queue))
    {
        /*Remove the first element from the front of the deque*/
        /*If the element is our destination, return 1, otherwise*/
        /*check & mark as visited and add it to the FRONT deque.*/
        ...
        else if(!currVert->isVisited)
        {
            for(i = 0; i < currVert->numNeighbors; ++i)
            {
                if(!currVert->neighbors[i]->isVisited)
                {
                    addFrontCirListDeque(queue, currVert->neighbors[i]);/*this makes it DFS*/
                    currVert->neighbors[i]->isVisited = 1;
                }
            }
            currVert->isVisited = 1;
        }
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    while(!isEmptyCirListDeque(queue))
    {
        /*Remove the first element from the front of the deque*/
        /*If the element is our destination, return 1, otherwise*/
        /*check & mark as visited and add it to the BACK of the deque.*/
        currVert = frontCirListDeque(queue);
        removeFrontCirListDeque(queue);
        if(currVert == destination)
        {
        ...
        else if(!currVert->isVisited)
        {
            for(i = 0; i < currVert->numNeighbors; ++i)
            {
                if(!currVert->neighbors[i]->isVisited)
                {
                    addBackCirListDeque(queue, currVert->neighbors[i]);/*this makes it a BFS*/
                    currVert->neighbors[i]->isVisited = 1;
                }
            }
            currVert->isVisited = 1;
